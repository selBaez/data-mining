...
Secondly, during the transductor stage we have one neural network, as described in Section~\ref{sec:transductor} and visualized in Figure~\ref{fig:nn_trans_design}. We set the required thresholds for the agreement and correlation tests to $ks_threshold = 0.08$ and $cvm_threshold=0.002$. It is worth mentioning that first we tried a threshold of $ks_threshold = 0.9$, but it was not sufficient to pass the competition's regulations. therefore, a lower value was applied to latter experiments.

Finally, we implement a stopping condition for producing only $30$ models, one at a time, which create predictions in order to try to pass the agreement and correlation test.
...
\begin{figure}[!ht]
	...
	\caption{Performance of the ensemble of neural networks with $0$ Dropout rate}
	\label{fig:dropout_0}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Performance of the ensemble of neural networks with initial Dropout rate of $22$}
...
Given these static settings, we first experimented with different Dropout rates during the ensemble phase.  The rates tested were $0, 7, 11, and 22$. Each initial rate was reduced at consecutive hidden layers at a rate of $2$. For example, if an initial rate of $22$ was provided, subsequent layers had rates of $20, 18$ and so on. A special heterogeneous case was tested with rates $22, 14.5$ and $7$ applied to the dynamic layers.
...
Secondly, we tested the length of pre-training for the transductor phase. To do so, the number of epochs for pre-training ranged within $1, 10, 20$ and $30$.

Figures~\ref{fig:transductor_1} and \ref{fig:transductor_30} show the contributions of the AUC, agreement metric (KS), correlation metric (CVM) have to the aggregated loss function against the iterations of minimization. No evidence for significant improvement is detected when increasing the length of pre-training, however, no conclusion can be reached and further experimentation might be needed.
...
Lastly, we experimented with changing the optimization algorithm used to train the model. Figure~\ref{fig:ensemble_d11} shows the performance of the ensemble of models
...
\section{Results}
\label{sec:results}
In this Section we present the final scores and rank obtained on the test set after submission to Kaggle. 

The highest score that we registered is $0.976174$ on the public leaderboard and $0.975412$ on the private one. Over all the trials that passed the correlation and agreement tests (18 in total) the mean is $0.96984$ and the standard deviation is $0.00461$. Those results would have given us a rank in the second half of 500, between the score achieved by Hierarchical Gradient Boosting (benchmark for the competition at rank 570-571, used for actual $\tau \rightarrow 3\mu$ analysis at LHCb) with a score of $0.976709$ and Gradient Boosting (another benchmark ranked at 587-588) with a score of
$0.971025$.

- objective not achieved. No comparable performance achieved


\section{Conclusion}
\label{sec:conclusion}
DOmain knowledge is valuable and sometimes needed. Blind mining lead to good results but no understanding.
